...
def generate_launch_description():
    # planning_context
    robot_description_config = os.path.join(get_package_share_directory('bravo_7_description'), 'urdf', 'bravo_7_robot.xacro')
    ...
    declare_model_cmd = DeclareLaunchArgument(
            'model',
            default_value='bravo_7_robot.xacro',
    ...
    oru_moveit_path = os.path.join(
        get_package_share_directory('bravo_7_moveit_config'), 'launch/robot')

    moveit_servo = IncludeLaunchDescription(
        AnyLaunchDescriptionSource(os.path.join(
            oru_moveit_path, 'bravo_7_servo_teleop.launch.py'))
    )
    ...
    switch_controller_file_name = 'switch_controller.yaml'

    switch_controller_file_path = os.path.join(
        get_package_share_directory('bravo_7_bringup'),
        'config',
        switch_controller_file_name)

    # MoveGroupInterface demo executable
    switch_controller_node = Node(package='pj_moveit2_adaptor',
                               executable='switch_controllers.py',
                               output='screen',
                               parameters=[switch_controller_file_path],
                               respawn=True, 
                               respawn_delay=2.0,
