...
{
StructPack::StructPack(const char * format, va_list inArg) : format(format)
{
  va_copy(arg, inArg);
  va_end(inArg);
}
...
StructPack::~StructPack() { va_end(arg); }
    ...
    if (width > 0) {
      switch (*format) {
        case 'B': {
          ...
          while (width--) pack_uint8(*ptr++);
          break;
        }
        case 'L': {
          uint32_t * ptr = va_arg(arg, uint32_t *);
          while (width--) pack_uint32(*ptr++);
          break;
        }
        case 'f': {
          float * ptr = va_arg(arg, float *);
          while (width--) pack_float(*ptr++);
          break;
        }
        case 's': {
      ...
      switch (*format) {
        case 'B':
          pack_uint8(va_arg(arg, unsigned int));
          break;
        case 'L':
          pack_uint32(va_arg(arg, uint32_t));
          break;
        case 'f':
          pack_float(va_arg(arg, double));
          break;
      }
    }
  }
...
void StructPack::pack_uint8(uint8_t val) { emit(val); }
  ...
  for (; *format != 0; format++) {
    ...
    if (width > 0) {
      switch (*format) {
        case 'B': {
          uint8_t * ptr = va_arg(arg, uint8_t *);
          while (width--) unpack_uint8(ptr++);
          break;
        }
        case 'L': {
          uint32_t * ptr = va_arg(arg, uint32_t *);
          while (width--) unpack_uint32(ptr++);
          break;
        }
        case 'f': {
          float * ptr = va_arg(arg, float *);
          while (width--) unpack_float(ptr++);
          break;
        }
        case 's':
          str = va_arg(arg, char *);
...
void StructPack::unpack_uint8(uint8_t * val) { *val = fetch(); }
void StructPack::unpack_uint32(uint32_t * val)
{
  *val = (fetch() << 0) | (fetch() << 8) | (fetch() << 16) | (fetch() << 24);
}
